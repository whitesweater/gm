...
def skgeom_dist(p1, p2):
    return np.sqrt(
        (float(p1.x()) - float(p2.x())) ** 2 + (float(p1.y()) - float(p2.y())) ** 2
    )
    ...
    for h in skeleton.halfedges:
        if h.is_bisector:
            ...
            if not (is_on_contour(polygon, p1) and is_on_contour(polygon, p2)):
                G.add_edge(
                    h.vertex.id, h.opposite.vertex.id, weight=skgeom_dist(p1, p2)
                )
...
def get_size_with_vector(mrr):
    ...
    if axis1 <= axis2:
        return (
            ...
            np.array(bbox[0]) - np.array(bbox[3]),
            [0, 1],
            [0, 3],
        )
    else:
...
def get_block_width_from_pt_on_midaxis(block, vector_midaxis, pt_on_midaxis):
    ...
    left_line_to_contour = get_extend_line(
        dummy_left_pt, pt_on_midaxis, block, False, is_extend_from_end=True
    )
    right_line_to_contour = get_extend_line(
        dummy_right_pt, pt_on_midaxis, block, False, is_extend_from_end=True
    )
...
def warp_bldg_by_midaxis(bldg, block, midaxis):
    bldgnum = len(bldg)  # 建筑物的数量
    normalized_size = []  # 存储归一化后的建筑物大小
    midaxis_length = midaxis.length  # 街区中轴线的长度

    ################################################################
    relative_cutoff = [0.0]  # 储街区中轴线上每个点相对于中轴线起点的归一化距离
    vector_midaxis = []  # 中轴线上每两个相邻点之间的向量
    block_width_list = []  # 存储街区中轴线上每个点的宽度

    coords = np.array(midaxis.coords.xy)
    # coords.shape[1]表示的点的数量
    for i in range(1, coords.shape[1]):
        relative_cutoff.append(
            midaxis.project(Point(coords[0, i], coords[1, i]), normalized=True)
        )
        vector_midaxis.append(coords[:, i] - coords[:, i - 1])
        if i < coords.shape[1] - 1:
            cur_width = get_block_width_from_pt_on_midaxis(
                block,
                coords[:, i] - coords[:, i - 1],
                Point(coords[0, i], coords[1, i]),
            )  # each node on midaxis, except the last and the front.
    ...
    normalized_y = []
    for i in range(bldgnum):
        vertical_point_on_midaxis = midaxis.interpolate(
            normalized_x[i], normalized=True
        )

        vector_midaxis_to_bldg = np.array(
            [
                bldg[i].centroid.x - vertical_point_on_midaxis.x,
                bldg[i].centroid.y - vertical_point_on_midaxis.y,
            ]
        )
        ...
        dist_from_midaxis_to_bldg = np.sqrt(
            vector_midaxis_to_bldg[0] * vector_midaxis_to_bldg[0]
            + vector_midaxis_to_bldg[1] * vector_midaxis_to_bldg[1]
        )

        relative_y = (
                2.0 * dist_from_midaxis_to_bldg / mean_block_width
        )  # changed from "dist_from_midaxis_to_contour", without 2.0 multiple
        if cross_prod <= 0:
        ...
        ###########################################################################################
        # 最小外接矩形
        longside, shortside, long_vec, short_vec, _, _ = get_size_with_vector(
            bldg[i].minimum_rotated_rectangle
        )
        long_vec = long_vec / np.linalg.norm(long_vec)
        unit_cur_vector_midaxis = cur_vector_midaxis / np.linalg.norm(
            cur_vector_midaxis
        )
        ...
        normalized_size_x = (
                2 * longside / midaxis_length
        )  ############ because pos is [-1, 1], size can be 2
        normalized_size_y = (
                2 * shortside / mean_block_width
        )  # changed from divded by "dist_from_midaxis_to_contour"
        normalized_size.append([normalized_size_x, normalized_size_y])

    normalized_x = np.array(normalized_x, dtype=np.double)
    normalized_x = (
            2.0 * normalized_x - 1.0
    )  ############ normalize pos_x to [-1, 1], pos_y already been [-1, 1]
...
def move_to_origin_efficient(polygons, polygons2):
    centroid = polygons.centroid
    moved_polygons = []
    for polygon in polygons2:
        moved_polygons.append(translate(polygon, -centroid.x, -centroid.y))

    return translate(polygons, -centroid.x, -centroid.y), moved_polygons
